package com.scene.baselib.pull_loadmore;
...
import com.scene.baselib.pull_loadmore.indicator.PtrIndicator;
import com.scene.baselib.pull_loadmore.loadmore.DefaultLoadMoreFooter;
import com.scene.baselib.pull_loadmore.loadmore.GridViewHandler;
import com.scene.baselib.pull_loadmore.loadmore.ILoadViewMoreFactory;
import com.scene.baselib.pull_loadmore.loadmore.ILoadViewMoreFactory.ILoadMoreView;
import com.scene.baselib.pull_loadmore.loadmore.ListViewHandler;
import com.scene.baselib.pull_loadmore.loadmore.OnLoadMoreListener;
import com.scene.baselib.pull_loadmore.loadmore.OnScrollBottomListener;
import com.scene.baselib.pull_loadmore.loadmore.RecyclerViewHandler;
import com.scene.baselib.pull_loadmore.utils.PtrCLog;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, com.scene.baselib.R.styleable.PtrFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(com.scene.baselib.R.styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
